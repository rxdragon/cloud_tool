<template>
  <div>
    <transition name="main">
      <div id="ImgPreview" ref="ImgPreview" v-show="viewerVisibleShow" @mousewheel="stopEvent">
        <!-- 预览窗的title-->
        <div class="titleBar" ref="titleBar">
          <div class="imgName">{{ imgName }}</div>
          <div><img src="./img/close.png" class="closeBtn" @click="visible()" /></div>
        </div>
        <!--左右按钮-->
        <div class="pre" ref="preCover"><img src="./img/pre.png" @click="prevImg" ref="preBtn" /></div>

        <div class="next" ref="nextCover"><img src="./img/next.png" @click="nextImg" ref="nextBtn" /></div>
        ...
        <div class="toolBar" ref="toolBar">
          <div class="btnGroup">
            <div><img src="./img/rotate.png" @click="rotateImg" /></div>

            <div><img src="./img/reset.png" @click="reset" /></div>

            <div><img src="./img/download.png" @click="download(imgIndex)" /></div>
          </div>
        </div>
        <!--end-->
      </div>
...
export default class ImgPreview extends Vue {
  ...
  public mousewheel (e: any) {
    ...
    if (e.target && this.error === false && e.target === this.imgElement) {
      ...
      snapScaleSize = snapScaleSize < 0.2 ? 0.2 : snapScaleSize > 5 ? 5 : snapScaleSize // 可以缩小到0.2,放大到5倍
      // 计算位置，以鼠标所在位置为中心
      // 以每个点的x、y位置，计算其相对于图片的位置，再计算其相对放大后的图片的位置
      this.locationData.distanceX =
        this.locationData.distanceX - ((x - this.locationData.distanceX) * (snapScaleSize - this.scaleSize)) / this.scaleSize
      this.locationData.distanceY =
        this.locationData.distanceY - ((y - this.locationData.distanceY) * (snapScaleSize - this.scaleSize)) / this.scaleSize
  ...
  public initImgMsg (index: number) {
    ...
    this.imgContainerWidth = this.preloadImgData[index].width / 1
    this.imgContainerHeight = this.preloadImgData[index].height / 1
    ...
    // 初始化图片的宽高
    this.imgWidth = this.preloadImgData[index].width / 1
    this.imgHeight = this.preloadImgData[index].height / 1
    // 初始化图片的src和name
